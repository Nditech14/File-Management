...
namespace Template.Infrastruture.Repositories;
    ...
    public  async Task<IEnumerable<FileEntity>> GetFilesAsync()
    {
        ...
        await foreach (var pathItem in paths)
        {
            files.Add(new FileEntity
            {
                Id = pathItem.Name,
                Name = pathItem.Name,
                FilePath = _fileSystemClient.GetFileClient(pathItem.Name).Uri.AbsoluteUri
            });
        }
    ...
    public async Task<FileEntity> UploadFileAsync(Stream fileStream, string fileName)
    {
        try
        {
            ...
            return new FileEntity
            {
                Id = fileClient.Path,
                Name = fileName,
                FilePath = fileClient.Uri.AbsoluteUri,
                Size = properties.Value.ContentLength, // Correctly get the size from properties
